...
Для оценки производительности приложения было проведено тестирование на трёх наборах контактной базы: 100, 500 и 1000 записей. Измерялось время импорта CSV-файла, создания шаблона, создания кампании и подготовки кампании к отправке. Результаты представлены в таблице:

| Количество контактов | Время импорта CSV, сек | Время создания шаблона, сек | Время создания кампании, сек | Время подготовки кампании, сек | Количество созданных сообщений |
|--------------------|----------------------|----------------------------|-----------------------------|--------------------------------|-------------------------------|
| 100                | 0,354                | 0,044                      | 0,042                       | 0,227                          | 100                           |
| 500                | 1,543                | 0,027                      | 0,034                       | 0,632                          | 500                           |
| 1000               | 2,731                | 0,011                      | 0,027                       | 0,885                          | 1000                         |

Как видно из таблицы, основное увеличение времени наблюдается при импорте CSV-файлов, что логично для обработки большого объёма данных. Время подготовки кампании увеличивается умеренно и остаётся в пределах одной секунды даже при 1000 контактов. Остальные операции практически не зависят от размера базы.  

Эти результаты демонстрируют устойчивость приложения при работе с базой до 1000 контактов и подтверждают корректность реализации основных функциональных сценариев.
